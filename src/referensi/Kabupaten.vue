<template>
  <app>
    <v-row>
      <v-col cols="8">
        <span class="title pl-2">Kabupaten</span>
      </v-col>
      <v-spacer></v-spacer>
    ...
    >
      ...
      <template v-slot:item.provinsi_id="{ item }">{{item.provinsi.nama_provinsi}}</template>
    ...
    <v-dialog v-model="dialog.alert.model" max-width="500">
      <v-card>
        <v-card-title class="title">Apakah anda yakin ingin hapus data Kabupaten ini?</v-card-title>
        ...
        <v-card-text>
          <v-row>
            ...
            <v-col cols="11">
              ...
              anda yakin ingin menghapus data Kabupaten ini?
            </v-col>
          </v-row>
        </v-card-text>
        <v-divider></v-divider>
    ...
    <v-dialog v-model="dialog.form.model" persistent width="750">
      <v-card>
        <v-card-title primary-title>
          <v-icon class="pr-2">{{ icon_form }}</v-icon>Input Data Kabupaten
          <v-spacer></v-spacer>
            ...
            <v-row>
              ...
              <v-col cols="4">
                <v-text-field
                  ...
                  label="ID *"
                  hint="Contoh : 3273"
                  :prefix="form.isEdit ? '' : form.provinsi_id ? form.provinsi_id : ''"
                ></v-text-field>
              </v-col>
              <v-col cols="8">
                <v-text-field
        ...
        <v-card-actions>
          ...
          <v-btn
            ...
            text
            @click="form.isEdit ? updateNewKabupaten() : createNewKabupaten()"
          >
          ...
          <v-btn
            ...
            :disabled="dialog.form.loading"
            text
            @click="form.isEdit ? updateKabupaten() : createKabupaten()"
          >
...
export default {
  ...
  methods: {
    ...
    editItem(item) {
      ...
      this.form.nama_kabupaten = item.nama_kabupaten;
    },
    destroyItem(item) {
    ...
    filterDataProvinsi(item) {
      const provinsi = this.select.provinsi;
      return provinsi.find(filter => {

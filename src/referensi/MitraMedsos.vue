<template>
  <app>
    <v-row>
      <v-col cols="8">
        <span class="title pl-2">MitraMedsos</span>
      </v-col>
      <v-spacer></v-spacer>
    ...
    <v-dialog v-model="dialog.alert.model" max-width="500">
      <v-card>
        <v-card-title class="title">Apakah anda yakin ingin hapus data MitraMedsos ini?</v-card-title>
        ...
        <v-card-text>
          <v-row>
            ...
            <v-col cols="11">
              ...
              anda yakin ingin menghapus data MitraMedsos ini?
            </v-col>
          </v-row>
        </v-card-text>
        <v-divider></v-divider>
    ...
    <v-dialog v-model="dialog.form.model" persistent width="750">
      <v-card>
        <v-card-title primary-title>
          <v-icon class="pr-2">{{ icon_form }}</v-icon>Input Data MitraMedsos
          <v-spacer></v-spacer>
        ...
        <v-card-actions>
          ...
          <v-btn
            ...
            text
            @click="
              form.isEdit ? updateNewMitraMedsos() : createNewMitraMedsos()
            "
          >
          ...
          <v-btn
            ...
            :disabled="dialog.form.loading"
            text
            @click="form.isEdit ? updateMitraMedsos() : createMitraMedsos()"
          >
...
export default {
  ...
  watch: {
    "table.options"(newValue, oldValue) {
      this.fetchMitraMedsos();
    },
    search(newValue) {
      this.table.options.page = 1;
      this.fetchMitraMedsos(newValue);
    }
  },
  computed: {
    search() {
  ...
  methods: {
    fetchMitraMedsos() {
      ...
      const params = data;
      this.axios
        .get("mitramedsos", { params })
        .then(response => {
          this.table.items = response.data.mitramedsos;
        })
        .catch(error => {
    ...
    updateMitraMedsos() {
      ...
      this.axios
        .post("mitramedsos/update", params)
        .then(response => {
    ...
    createMitraMedsos() {
      const params = this.form;
      this.axios
        .post("mitramedsos/create", params)
        .then(response => {
          this.table.items = response.data;
          this.dialog.form.model = false;
          this.table.options.page = 1;

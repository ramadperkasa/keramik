<template>
  <app>
    <v-row>
      <v-col cols="8">
        <span class="title pl-2">KonsumenRekening</span>
      </v-col>
      <v-spacer></v-spacer>
    ...
    <v-dialog v-model="dialog.alert.model" max-width="500">
      <v-card>
        <v-card-title class="title">
          Apakah anda yakin ingin hapus data KonsumenRekening
          ini?
        </v-card-title>
        ...
        <v-card-text>
          <v-row>
            ...
            <v-col cols="11">
              ...
              anda yakin ingin menghapus data KonsumenRekening ini?
            </v-col>
          </v-row>
        </v-card-text>
        <v-divider></v-divider>
    ...
    <v-dialog v-model="dialog.form.model" persistent width="750">
      <v-card>
        <v-card-title primary-title>
          <v-icon class="pr-2">{{ icon_form }}</v-icon>Input Data KonsumenRekening
          <v-spacer></v-spacer>
        ...
        <v-container>
          <v-card-text>
            ...
            ></v-text-field>
            <v-text-field v-model="form.bank_id" label="Bank Id *" hint="Contoh : Dipointer"></v-text-field>
            <v-text-field
        ...
        <v-card-actions>
          ...
          <v-btn
            ...
            @click="
              form.isEdit
                ? updateNewKonsumenRekening()
                : createNewKonsumenRekening()
            "
          >
          ...
          <v-btn
            ...
            text
            @click="
              form.isEdit ? updateKonsumenRekening() : createKonsumenRekening()
            "
          >
...
export default {
  ...
  watch: {
    "table.options"(newValue, oldValue) {
      this.fetchKonsumenRekening();
    },
    search(newValue) {
      this.table.options.page = 1;
      this.fetchKonsumenRekening(newValue);
    }
  },
  computed: {
    search() {
  ...
  methods: {
    fetchKonsumenRekening() {
      ...
      const params = data;
      this.axios
        .get("konsumenrekening", { params })
        .then(response => {
          this.table.items = response.data.konsumenrekening;
        })
        .catch(error => {
    ...
    updateKonsumenRekening() {
      this.dialog.form.loading = true;
      const params = this.form;
      this.axios
        .post("konsumenrekening/update", params)
        .then(response => {
    ...
    createKonsumenRekening() {
      const params = this.form;
      this.axios
        .post("konsumenrekening/create", params)
        .then(response => {
          this.table.items = response.data;
          this.dialog.form.model = false;
          this.table.options.page = 1;
    ...
    destroyKonsumenRekening() {
      this.dialog.alert.loading = true;
      const id = this.form.id;
      const konsumen_id = this.form.konsumen_id;
      this.axios
        .post("konsumenrekening/destroy", { id })
        .then(response => {
          this.table.items = response.data;
          this.table.options.page = 1;
          this.alert.model = true;

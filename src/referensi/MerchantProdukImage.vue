<template>
  <app>
    <v-row>
      <v-col cols="8">
        <span class="title pl-2">MerchantProdukImage</span>
      </v-col>
      <v-spacer></v-spacer>
    ...
    <v-dialog v-model="dialog.alert.model" max-width="500">
      <v-card>
        <v-card-title class="title">
          Apakah anda yakin ingin hapus data MerchantProdukImage
          ini?
        </v-card-title>
        ...
        <v-card-text>
          <v-row>
            ...
            <v-col cols="11">
              ...
              anda yakin ingin menghapus data MerchantProdukImage ini?
            </v-col>
          </v-row>
        </v-card-text>
        <v-divider></v-divider>
        <v-card-actions>
          ...
          <v-btn
            ...
            :disabled="dialog.alert.loading"
            text
            @click="destroyMerchantProdukImage()"
          >Hapus</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="dialog.form.model" persistent width="750">
      <v-card>
        <v-card-title primary-title>
          <v-icon class="pr-2">{{ icon_form }}</v-icon>Input Data MerchantProdukImage
          <v-spacer></v-spacer>
        ...
        <v-container>
          <v-card-text>
            ...
            <v-text-field v-model="form.file_jenis" label="File Jenis *" hint="Contoh : Dipointer"></v-text-field>
            <v-text-field v-model="form.file_nama" label="File Nama *" hint="Contoh : Dipointer"></v-text-field>
          </v-card-text>
...
export default {
  ...
  watch: {
    "table.options"(newValue, oldValue) {
      this.fetchMerchantProdukImage();
    },
    search(newValue) {
      this.table.options.page = 1;
      this.fetchMerchantProdukImage(newValue);
    }
  },
  computed: {
    search() {
  ...
  methods: {
    ...
    editItem(item) {
      ...
      this.form.file_jenis = item.file_jenis;
      this.form.file_nama = item.file_nama;
    },
    destroyItem(item) {

<template>
  <app>
    <v-row>
      <v-col cols="8">
        <span class="title pl-2">KonsumenMedsos</span>
      </v-col>
      <v-spacer></v-spacer>
    ...
    <v-dialog v-model="dialog.alert.model" max-width="500">
      <v-card>
        <v-card-title class="title">Apakah anda yakin ingin hapus data KonsumenMedsos ini?</v-card-title>
        ...
        <v-card-text>
          <v-row>
            ...
            <v-col cols="11">
              ...
              anda yakin ingin menghapus data KonsumenMedsos ini?
            </v-col>
          </v-row>
        </v-card-text>
        <v-divider></v-divider>
    ...
    <v-dialog v-model="dialog.form.model" persistent width="750">
      <v-card>
        <v-card-title primary-title>
          <v-icon class="pr-2">{{ icon_form }}</v-icon>Input Data KonsumenMedsos
          <v-spacer></v-spacer>
        ...
        <v-card-actions>
          ...
          <v-btn
            ...
            text
            @click="
              form.isEdit ? updateKonsumenMedsos() : createKonsumenMedsos()
            "
          >
...
export default {
  ...
  watch: {
    "table.options"(newValue, oldValue) {
      this.fetchKonsumenMedsos();
    },
    search(newValue) {
      this.table.options.page = 1;
      this.fetchKonsumenMedsos(newValue);
    }
  },
  computed: {
    search() {

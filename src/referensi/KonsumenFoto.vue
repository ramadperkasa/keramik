<template>
  <app>
    <v-row>
      <v-col cols="8">
        <span class="title pl-2">KonsumenFoto</span>
      </v-col>
      <v-spacer></v-spacer>
    ...
    <v-dialog v-model="dialog.alert.model" max-width="500">
      <v-card>
        <v-card-title class="title">Apakah anda yakin ingin hapus data KonsumenFoto ini?</v-card-title>
        ...
        <v-card-text>
          <v-row>
            ...
            <v-col cols="11">
              ...
              anda yakin ingin menghapus data KonsumenFoto ini?
            </v-col>
          </v-row>
        </v-card-text>
        <v-divider></v-divider>
    ...
    <v-dialog v-model="dialog.form.model" persistent width="750">
      <v-card>
        <v-card-title primary-title>
          <v-icon class="pr-2">{{ icon_form }}</v-icon>Input Data KonsumenFoto
          <v-spacer></v-spacer>
        ...
        <v-container>
          <v-card-text>
            ...
            ></v-text-field>
            <v-text-field v-model="form.nama_file" label="Nama File *" hint="Contoh : Dipointer"></v-text-field>
            <v-text-field v-model="form.path" label="Path *" hint="Contoh : Dipointer"></v-text-field>
            <v-text-field v-model="form.jenis_file" label="Jenis File *" hint="Contoh : Dipointer"></v-text-field>
          </v-card-text>
        ...
        <v-card-actions>
          ...
          <v-btn
            ...
            text
            @click="
              form.isEdit ? updateNewKonsumenFoto() : createNewKonsumenFoto()
            "
          >
          ...
          <v-btn
            ...
            :disabled="dialog.form.loading"
            text
            @click="form.isEdit ? updateKonsumenFoto() : createKonsumenFoto()"
          >
...
export default {
  ...
  watch: {
    "table.options"(newValue, oldValue) {
      this.fetchKonsumenFoto();
    },
    search(newValue) {
      this.table.options.page = 1;
      this.fetchKonsumenFoto(newValue);
    }
  },
  computed: {
    search() {
  ...
  methods: {
    fetchKonsumenFoto() {
      ...
      const params = data;
      axios
        .get("konsumenfoto", { params })
        .then(response => {
          this.table.items = response.data.konsumenfoto;
        })
        .catch(error => {
    ...
    editItem(item) {
      ...
      this.form.nama_file = item.nama_file;
      this.form.path = item.path;
      this.form.jenis_file = item.jenis_file;
    },
    destroyItem(item) {
    ...
    updateKonsumenFoto() {
      ...
      axios
        .post("konsumenfoto/update", params)
        .then(response => {
    ...
    createKonsumenFoto() {
      const params = this.form;
      axios
        .post("konsumenfoto/create", params)
        .then(response => {
          this.table.items = response.data;
          this.dialog.form.model = false;
          this.table.options.page = 1;

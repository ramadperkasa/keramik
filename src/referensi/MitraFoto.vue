<template>
  <app>
    <v-row>
      <v-col cols="8">
        <span class="title pl-2">MitraFoto</span>
      </v-col>
      <v-spacer></v-spacer>
    ...
    <v-dialog v-model="dialog.alert.model" max-width="500">
      <v-card>
        <v-card-title class="title">Apakah anda yakin ingin hapus data MitraFoto ini?</v-card-title>
        ...
        <v-card-text>
          <v-row>
            ...
            <v-col cols="11">
              ...
              anda yakin ingin menghapus data MitraFoto ini?
            </v-col>
          </v-row>
        </v-card-text>
        <v-divider></v-divider>
    ...
    <v-dialog v-model="dialog.form.model" persistent width="750">
      <v-card>
        <v-card-title primary-title>
          <v-icon class="pr-2">{{ icon_form }}</v-icon>Input Data MitraFoto
          <v-spacer></v-spacer>
        ...
        <v-container>
          <v-card-text>
            ...
            <v-text-field
              v-model="form.jenis_foto_id"
              label="Jenis Foto Id *"
              hint="Contoh : Dipointer"
            ></v-text-field>
            <v-text-field v-model="form.foto_id" label="Foto Id *" hint="Contoh : Dipointer"></v-text-field>
            <v-text-field v-model="form.nama_foto" label="Nama Foto *" hint="Contoh : Dipointer"></v-text-field>
          </v-card-text>
        ...
        <v-card-actions>
          ...
          <v-btn
            ...
            text
            @click="form.isEdit ? updateNewMitraFoto() : createNewMitraFoto()"
          >
          ...
          <v-btn
            ...
            :disabled="dialog.form.loading"
            text
            @click="form.isEdit ? updateMitraFoto() : createMitraFoto()"
          >
...
export default {
  ...
  methods: {
    fetchMitraFoto() {
      ...
      const params = data;
      axios
        .get("mitrafoto", { params })
        .then(response => {
          this.table.items = response.data.mitrafoto;
        })
        .catch(error => {
    ...
    editItem(item) {
      ...
      this.form.jenis_foto_id = item.jenis_foto_id;
      this.form.foto_id = item.foto_id;
      this.form.nama_foto = item.nama_foto;
    },
    destroyItem(item) {

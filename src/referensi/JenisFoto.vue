<template>
  <app>
    <v-row>
      <v-col cols="8">
        <span class="title pl-2">JenisFoto</span>
      </v-col>
      <v-spacer></v-spacer>
    ...
    <v-dialog v-model="dialog.alert.model" max-width="500">
      <v-card>
        <v-card-title class="title">Apakah anda yakin ingin hapus data JenisFoto ini?</v-card-title>
        ...
        <v-card-text>
          <v-row>
            ...
            <v-col cols="11">
              ...
              anda yakin ingin menghapus data JenisFoto ini?
            </v-col>
          </v-row>
        </v-card-text>
        <v-divider></v-divider>
    ...
    <v-dialog v-model="dialog.form.model" persistent width="750">
      <v-card>
        <v-card-title primary-title>
          <v-icon class="pr-2">{{ icon_form }}</v-icon>Input Data JenisFoto
          <v-spacer></v-spacer>
        ...
        <v-card-actions>
          ...
          <v-btn
            ...
            text
            @click="form.isEdit ? updateNewJenisFoto() : createNewJenisFoto()"
          >
          ...
          <v-btn
            ...
            :disabled="dialog.form.loading"
            text
            @click="form.isEdit ? updateJenisFoto() : createJenisFoto()"
          >
...
export default {
  ...
  methods: {
    fetchJenisFoto() {
      ...
      const params = data;
      this.axios
        .get("jenisfoto", { params })
        .then(response => {
          this.table.items = response.data.jenisfoto;
        })
        .catch(error => {

<template>
  <app>
    <v-row>
      <v-col cols="8">
        <span class="title pl-2">Agama</span>
      </v-col>
      <v-spacer></v-spacer>
    ...
    <v-dialog v-model="dialog.alert.model" max-width="500">
      <v-card>
        <v-card-title class="title">Apakah anda yakin ingin hapus data Agama ini?</v-card-title>
        ...
        <v-card-text>
          <v-row>
            ...
            <v-col cols="11">
              ...
              anda yakin ingin menghapus data Agama ini?
            </v-col>
          </v-row>
        </v-card-text>
        <v-divider></v-divider>
    ...
    <v-dialog v-model="dialog.form.model" persistent width="750">
      <v-card>
        <v-card-title primary-title>
          <v-icon class="pr-2">{{ icon_form }}</v-icon>Input Data Agama
          <v-spacer></v-spacer>
        ...
        <v-container>
          <v-card-text>
            <v-text-field v-model="form.nama_agama" label="Nama Agama *" hint="Contoh : Islam"></v-text-field>
          </v-card-text>
        ...
        <v-card-actions>
          ...
          <v-btn
            ...
            text
            @click="form.isEdit ? updateNewAgama() : createNewAgama()"
          >
          ...
          <v-btn
            ...
            :disabled="dialog.form.loading"
            text
            @click="form.isEdit ? updateAgama() : createAgama()"
          >
...
export default {
  ...
  methods: {
    ...
    addItem() {
      ...
      this.form.id = "";
      this.form.nama_agama = "";
    },
    editItem(item) {
      this.dialog.form.model = true;
      this.form.isEdit = true;
      this.form.id = item.id;
      this.form.nama_agama = item.nama_agama;
    },
    destroyItem(item) {

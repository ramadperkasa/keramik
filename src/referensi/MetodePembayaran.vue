<template>
  <app>
    <v-row>
      <v-col cols="8">
        <span class="title pl-2">Metode Pembayaran</span>
      </v-col>
      <v-spacer></v-spacer>
    ...
    <v-dialog v-model="dialog.alert.model" max-width="500">
      <v-card>
        <v-card-title class="title">
          Apakah anda yakin ingin hapus data Metode Pembayaran
          ini?
        </v-card-title>
        ...
        <v-card-text>
          <v-row>
            ...
            <v-col cols="11">
              ...
              anda yakin ingin menghapus data Metode Pembayaran ini?
            </v-col>
          </v-row>
        </v-card-text>
        <v-divider></v-divider>
    ...
    <v-dialog v-model="dialog.form.model" persistent width="750">
      <v-card>
        <v-card-title primary-title>
          <v-icon class="pr-2">{{ icon_form }}</v-icon>Input Data Metode Pembayaran
          <v-spacer></v-spacer>
        ...
        <v-card-actions>
          ...
          <v-btn
            ...
            text
            @click="
              form.isEdit ? updateMetodePembayaran() : createMetodePembayaran()
            "
          >
...
export default {
  ...
  watch: {
    "table.options"(newValue, oldValue) {
      this.fetchMetodePembayaran();
    },
    search(newValue) {
      this.table.options.page = 1;
      this.fetchMetodePembayaran(newValue);
    }
  },
  computed: {
    search() {
  ...
  methods: {
    ...
    addItem() {
      ...
      this.form.id = "";
      this.form.nama_metode = "";
    },
    editItem(item) {
      this.dialog.form.model = true;
      this.form.isEdit = true;
      this.form.id = item.id;
      this.form.nama_metode = item.nama_metode;
    },
    destroyItem(item) {

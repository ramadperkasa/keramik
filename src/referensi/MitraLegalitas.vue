<template>
  <app>
    <v-row>
      <v-col cols="8">
        <span class="title pl-2">MitraLegalitas</span>
      </v-col>
      <v-spacer></v-spacer>
    ...
    <v-dialog v-model="dialog.alert.model" max-width="500">
      <v-card>
        <v-card-title class="title">Apakah anda yakin ingin hapus data MitraLegalitas ini?</v-card-title>
        ...
        <v-card-text>
          <v-row>
            ...
            <v-col cols="11">
              ...
              anda yakin ingin menghapus data MitraLegalitas ini?
            </v-col>
          </v-row>
        </v-card-text>
        <v-divider></v-divider>
    ...
    <v-dialog v-model="dialog.form.model" persistent width="750">
      <v-card>
        <v-card-title primary-title>
          <v-icon class="pr-2">{{ icon_form }}</v-icon>Input Data MitraLegalitas
          <v-spacer></v-spacer>
        ...
        <v-card-actions>
          ...
          <v-btn
            ...
            @click="
              form.isEdit
                ? updateNewMitraLegalitas()
                : createNewMitraLegalitas()
            "
          >
          ...
          <v-btn
            ...
            text
            @click="
              form.isEdit ? updateMitraLegalitas() : createMitraLegalitas()
            "
          >
...
export default {
  ...
  watch: {
    "table.options"(newValue, oldValue) {
      this.fetchMitraLegalitas();
    },
    search(newValue) {
      this.table.options.page = 1;
      this.fetchMitraLegalitas(newValue);
    }
  },
  computed: {
    search() {
  ...
  methods: {
    fetchMitraLegalitas() {
      ...
      const params = data;
      this.axios
        .get("mitralegalitas", { params })
        .then(response => {
          this.table.items = response.data.mitralegalitas;
        })
        .catch(error => {
    ...
    updateMitraLegalitas() {
      this.dialog.form.loading = true;
      const params = this.form;
      this.axios
        .post("mitralegalitas/update", params)
        .then(response => {
    ...
    createMitraLegalitas() {
      const params = this.form;
      this.axios
        .post("mitralegalitas/create", params)
        .then(response => {
          this.table.items = response.data;
          this.dialog.form.model = false;
          this.table.options.page = 1;
    ...
    destroyMitraLegalitas() {
      this.dialog.alert.loading = true;
      const id = this.form.id;
      const mitra_id = this.form.mitra_id;
      this.axios
        .post("mitralegalitas/destroy", { id })
        .then(response => {
          this.table.items = response.data;
          this.table.options.page = 1;
          this.alert.model = true;

<template>
  <app>
    <v-row>
      <v-col cols="8">
        <span class="title pl-2">Metode Pengiriman</span>
      </v-col>
      <v-spacer></v-spacer>
    ...
    <v-dialog v-model="dialog.alert.model" max-width="500">
      <v-card>
        <v-card-title class="title">
          Apakah anda yakin ingin hapus data MetodePengiriman
          ini?
        </v-card-title>
        ...
        <v-card-text>
          <v-row>
            ...
            <v-col cols="11">
              ...
              anda yakin ingin menghapus data Metode Pengiriman ini?
            </v-col>
          </v-row>
        </v-card-text>
        <v-divider></v-divider>
    ...
    <v-dialog v-model="dialog.form.model" persistent width="750">
      <v-card>
        <v-card-title primary-title>
          <v-icon class="pr-2">{{ icon_form }}</v-icon>Input Data Metode Pengiriman
          <v-spacer></v-spacer>
        ...
        <v-card-actions>
          ...
          <v-btn
            ...
            text
            @click="
              form.isEdit ? updateMetodePengiriman() : createMetodePengiriman()
            "
          >
...
export default {
  ...
  watch: {
    "table.options"(newValue, oldValue) {
      this.fetchMetodePengiriman();
    },
    search(newValue) {
      this.table.options.page = 1;
      this.fetchMetodePengiriman(newValue);
    }
  },
  computed: {
    search() {

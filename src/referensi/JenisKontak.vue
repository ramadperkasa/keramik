<template>
  <app>
    <v-row>
      <v-col cols="8">
        <span class="title pl-2">Jenis Kontak</span>
      </v-col>
      <v-spacer></v-spacer>
    ...
    <v-dialog v-model="dialog.alert.model" max-width="500">
      <v-card>
        <v-card-title class="title">Apakah anda yakin ingin hapus data JenisKontak ini?</v-card-title>
        ...
        <v-card-text>
          <v-row>
            ...
            <v-col cols="11">
              ...
              anda yakin ingin menghapus data JenisKontak ini?
            </v-col>
          </v-row>
        </v-card-text>
        <v-divider></v-divider>
    ...
    <v-dialog v-model="dialog.form.model" persistent width="750">
      <v-card>
        <v-card-title primary-title>
          <v-icon class="pr-2">{{ icon_form }}</v-icon>Input Data JenisKontak
          <v-spacer></v-spacer>
        ...
        <v-container>
          <v-card-text>
            <v-text-field v-model="form.nama_jenis" label="Nama Jenis *" hint="Contoh : Whatsapp"></v-text-field>
          </v-card-text>
        ...
        <v-card-actions>
          ...
          <v-btn
            ...
            text
            @click="
              form.isEdit ? updateNewJenisKontak() : createNewJenisKontak()
            "
          >
          ...
          <v-btn
            ...
            :disabled="dialog.form.loading"
            text
            @click="form.isEdit ? updateJenisKontak() : createJenisKontak()"
          >
...
export default {
  ...
  watch: {
    "table.options"(newValue, oldValue) {
      this.fetchJenisKontak();
    },
    search(newValue) {
      this.table.options.page = 1;
      this.fetchJenisKontak(newValue);
    }
  },
  computed: {
    search() {

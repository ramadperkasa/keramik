<template>
  <app>
    <v-row>
      <v-col cols="8">
        <span class="title pl-2">Kecamatan</span>
      </v-col>
      <v-spacer></v-spacer>
    ...
    >
      ...
      <template v-slot:item.kabupaten_id="{ item }">{{item.kabupaten.nama_kabupaten}}</template>
    ...
    <v-dialog v-model="dialog.alert.model" max-width="500">
      <v-card>
        <v-card-title class="title">Apakah anda yakin ingin hapus data Kecamatan ini?</v-card-title>
        ...
        <v-card-text>
          <v-row>
            ...
            <v-col cols="11">
              ...
              anda yakin ingin menghapus data Kecamatan ini?
            </v-col>
          </v-row>
        </v-card-text>
        <v-divider></v-divider>
    ...
    <v-dialog v-model="dialog.form.model" persistent width="750">
      <v-card>
        <v-card-title primary-title>
          <v-icon class="pr-2">{{ icon_form }}</v-icon>Input Data Kecamatan
          <v-spacer></v-spacer>
            ...
            <v-row>
              ...
              <v-col cols="4">
                <v-text-field
                  ...
                  label="ID *"
                  hint="Contoh : 327306"
                  :prefix="form.isEdit == true ? '' : form.provinsi_id && !form.kabupaten_id ? form.provinsi_id : form.provinsi_id && form.kabupaten_id ? form.kabupaten_id : ''"
                ></v-text-field>
              </v-col>
              <v-col cols="8">
                <v-text-field
        ...
        <v-card-actions>
          ...
          <v-btn
            ...
            text
            @click="form.isEdit ? updateNewKecamatan() : createNewKecamatan()"
          >
          ...
          <v-btn
            ...
            :disabled="dialog.form.loading"
            text
            @click="form.isEdit ? updateKecamatan() : createKecamatan()"
          >
...
export default {
  ...
  methods: {
    ...
    editItem(item) {
      ...
      this.form.provinsi_id = item.kabupaten_id.substring(0, 2);
      this.form.kabupaten_id = item.kabupaten_id;
      this.form.nama_kecamatan = item.nama_kecamatan;
    },
    destroyItem(item) {

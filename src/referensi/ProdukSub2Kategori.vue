<template>
  <app>
    <v-row>
      <v-col cols="8">
        <span class="title pl-2">ProdukSub2Kategori</span>
      </v-col>
      <v-spacer></v-spacer>
    ...
    <v-dialog v-model="dialog.alert.model" max-width="500">
      <v-card>
        <v-card-title class="title"
          >Apakah anda yakin ingin hapus data ProdukSub2Kategori
          ini?</v-card-title
        >
        ...
        <v-card-text>
          <v-row>
            ...
            <v-col cols="11"
              ...
              anda yakin ingin menghapus data ProdukSub2Kategori ini?</v-col
            >
          </v-row>
        </v-card-text>
        <v-divider></v-divider>
...
export default {
  ...
  methods: {
    ...
    updateProdukSub2Kategori() {
      ...
      axios
        .post("produksub2kategori/update", params)
        .then(response => {
    ...
    createProdukSub2Kategori() {
      const params = this.form;
      axios
        .post("produksub2kategori/create", params)
        .then(response => {
          this.table.items = response.data;
          this.dialog.form.model = false;
          this.table.options.page = 1;
    ...
    destroyProdukSub2Kategori() {
      ...
      axios
        .post("produksub2kategori/destroy", { id })
        .then(response => {
          this.table.items = response.data;
          this.table.options.page = 1;
          this.alert.model = true;

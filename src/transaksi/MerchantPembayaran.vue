<template>
  <app>
    <v-row>
      <v-col cols="8">
        <span class="title pl-2">MerchantPembayaran</span>
      </v-col>
      <v-spacer></v-spacer>
    ...
    <v-dialog v-model="dialog.alert.model" max-width="500">
      <v-card>
        <v-card-title class="title"
          >Apakah anda yakin ingin hapus data MerchantPembayaran
          ini?</v-card-title
        >
        ...
        <v-card-text>
          <v-row>
            ...
            <v-col cols="11"
              ...
              anda yakin ingin menghapus data MerchantPembayaran ini?</v-col
            >
          </v-row>
        </v-card-text>
        <v-divider></v-divider>
        <v-card-actions>
          ...
          <v-btn
            ...
            :disabled="dialog.alert.loading"
            text
            @click="destroyMerchantPembayaran()"
            >Hapus</v-btn
          >
        </v-card-actions>
      </v-card>
      ...
      <v-card>
        ...
        <v-container>
          <v-card-text>
            ...
            ></v-text-field>
            <v-text-field
              v-model="form.order_jual_detail_id"
              label="Order Jual Detail Id *"
              hint="Contoh : Dipointer"
            ></v-text-field>
            <v-text-field
            ...
            ></v-text-field>
            <v-text-field
              v-model="form.rekening_merchant_id"
              label="Rekening Merchant Id *"
              hint="Contoh : Dipointer"
            ></v-text-field>
          </v-card-text>
        ...
        <v-card-actions>
          ...
          <v-btn
            ...
            @click="
              form.isEdit
                ? updateNewMerchantPembayaran()
                : createNewMerchantPembayaran()
            "
          >
          ...
          <v-btn
            ...
            @click="
              form.isEdit
                ? updateMerchantPembayaran()
                : createMerchantPembayaran()
            "
          >
...
export default {
    ...
    return {
      ...
      table: {
        ...
        headers: [
          ...
          {
            text: "Order Jual Detail Id",
            value: "order_jual_detail_id",
            align: "left"
          },
          {
            text: "Rekening Id",
          ...
          {
            text: "Rekening Merchant Id",
            value: "rekening_merchant_id",
            align: "left"
          },
          {
            text: "Action",
  ...
  watch: {
    "table.options"(newValue, oldValue) {
      this.fetchMerchantPembayaran();
    },
    search(newValue) {
      this.table.options.page = 1;
      this.fetchMerchantPembayaran(newValue);
    }
  },
  computed: {
    search() {
  ...
  methods: {
    fetchMerchantPembayaran() {
      ...
      const params = data;
      axios
        .get("merchantpembayaran", { params })
        .then(response => {
          this.table.items = response.data.merchantpembayaran;
        })
        .catch(error => {
    ...
    addItem() {
      ...
      this.form.order_jual_detail_id = "";
      this.form.rekening_id = "";
      this.form.rekening_merchant_id = "";
    },
    editItem(item) {
      this.dialog.form.model = true;
      this.form.isEdit = true;
      this.form.order_jual_id = item.order_jual_id;
      this.form.order_jual_detail_id = item.order_jual_detail_id;
      this.form.rekening_id = item.rekening_id;
      this.form.rekening_merchant_id = item.rekening_merchant_id;
    },
    destroyItem(item) {
    ...
    updateMerchantPembayaran() {
      ...
      axios
        .post("merchantpembayaran/update", params)
        .then(response => {
    ...
    createMerchantPembayaran() {
      const params = this.form;
      axios
        .post("merchantpembayaran/create", params)
        .then(response => {
          this.table.items = response.data;
          this.dialog.form.model = false;
          this.table.options.page = 1;
    ...
    destroyMerchantPembayaran() {
      this.dialog.alert.loading = true;
      const order_jual_id = this.form.order_jual_id;
      const order_jual_detail_id = this.form.order_jual_detail_id;
      axios
        .post("merchantpembayaran/destroy", {id})
        .then(response => {
          this.table.items = response.data;
          this.table.options.page = 1;
          this.alert.model = true;

<template>
  <app>
    <v-row>
      <v-col cols="8">
        <span class="title pl-2">OrderJualDetailPembarayan</span>
      </v-col>
      <v-spacer></v-spacer>
    ...
    <v-dialog v-model="dialog.alert.model" max-width="500">
      <v-card>
        <v-card-title class="title"
          >Apakah anda yakin ingin hapus data OrderJualDetailPembarayan
          ini?</v-card-title
        >
        ...
        <v-card-text>
          <v-row>
            ...
            <v-col cols="11"
              ...
              anda yakin ingin menghapus data OrderJualDetailPembarayan
              ini?</v-col
            >
          </v-row>
        </v-card-text>
        ...
        <v-card-actions>
          ...
          <v-btn
            ...
            :disabled="dialog.alert.loading"
            text
            @click="destroyOrderJualDetailPembarayan()"
            >Hapus</v-btn
          >
        </v-card-actions>
      </v-card>
    ...
    <v-dialog v-model="dialog.form.model" persistent width="750">
      <v-card>
        <v-card-title primary-title>
          ...
          >Input Data OrderJualDetailPembarayan
          <v-spacer></v-spacer>
        ...
        <v-container>
          <v-card-text>
            ...
            ></v-text-field>
            <v-text-field
              v-model="form.rekening_konsumen_id"
              label="Rekening Konsumen Id *"
              hint="Contoh : Dipointer"
            ></v-text-field>
          </v-card-text>
        ...
        <v-card-actions>
          ...
          <v-btn
            ...
            @click="
              form.isEdit
                ? updateNewOrderJualDetailPembarayan()
                : createNewOrderJualDetailPembarayan()
            "
          >
          ...
          <v-btn
            ...
            @click="
              form.isEdit
                ? updateOrderJualDetailPembarayan()
                : createOrderJualDetailPembarayan()
            "
          >
...
export default {
    ...
    return {
      ...
      table: {
        ...
        headers: [
          ...
          {
            text: "Rekening Konsumen Id",
            value: "rekening_konsumen_id",
            align: "left"
          },
          {
            text: "Action",
  ...
  watch: {
    "table.options"(newValue, oldValue) {
      this.fetchOrderJualDetailPembarayan();
    },
    search(newValue) {
      this.table.options.page = 1;
      this.fetchOrderJualDetailPembarayan(newValue);
    }
  },
  computed: {
    search() {
  ...
  methods: {
    fetchOrderJualDetailPembarayan() {
      ...
      const params = data;
      axios
        .get("orderjualdetailpembarayan", { params })
        .then(response => {
          this.table.items = response.data.orderjualdetailpembarayan;
        })
        .catch(error => {
    ...
    addItem() {
      ...
      this.form.rekening_konsumen_id = "";
    },
    editItem(item) {
      this.dialog.form.model = true;
      ...
      this.form.rekening_konsumen_id = item.rekening_konsumen_id;
    },
    destroyItem(item) {
    ...
    updateOrderJualDetailPembarayan() {
      ...
      axios
        .post("orderjualdetailpembarayan/update", params)
        .then(response => {
    ...
    createOrderJualDetailPembarayan() {
      const params = this.form;
      axios
        .post("orderjualdetailpembarayan/create", params)
        .then(response => {
          this.table.items = response.data;
          this.dialog.form.model = false;
          this.table.options.page = 1;
    ...
    destroyOrderJualDetailPembarayan() {
      this.dialog.alert.loading = true;
      const order_jual_id = this.form.order_jual_id;
      axios
        .post("orderjualdetailpembarayan/destroy", {id})
        .then(response => {
          this.table.items = response.data;
          this.table.options.page = 1;
          this.alert.model = true;
